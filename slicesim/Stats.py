...
class Stats:
    ...
    def get_total_used_bw(self):
        t = 0
        for bs in self.base_stations:
            for sl in self._get_slices(bs):
                if not hasattr(sl, "capacity") or not isinstance(sl.capacity, Container):
                    print("BAD SLICE:", sl, type(sl), getattr(sl, "capacity", None))
                    continue  # Skip bad slices
    ...
    def get_avg_slice_client_count(self):
        ...
        for bs in self.base_stations:
            for sl in self._get_slices(bs):
                if not hasattr(sl, "connected_users"):
                    continue
                c += 1
                t += sl.connected_users
        return t/c if c != 0 else 0
    ...
    def is_client_in_coverage(self, client):
        if isinstance(self.area, (list, tuple)) and len(self.area) == 2:
            x_range, y_range = self.area
            if isinstance(x_range, (list, tuple)) and isinstance(y_range, (list, tuple)):
                return x_range[0] <= client.x <= x_range[1] and y_range[0] <= client.y <= y_range[1]
            elif isinstance(x_range, (int, float)) and isinstance(y_range, (int, float)):
            # Interpret area as (width, height) from origin (0,0)
                return 0 <= client.x <= x_range and 0 <= client.y <= y_range
        return False  # fallback in case of misconfigured area
