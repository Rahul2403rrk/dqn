...
class Client:
    def __init__(self, pk, env, x, y, mobility_pattern,
        ...
        self.action = None  # ✅ Will be started later in __main__.py
    ...
    def iter(self):
        ...
        # .75: Move
        x, y = self.mobility_pattern.generate_movement() if self.mobility_pattern else (0, 0)
        self.x += x
        self.y += y
    ...
    def generate_usage_and_connect(self):
        if self.usage_freq < random.random() and self.get_slice() is not None:
            ...
            self.connect()
            print(f'Client_{self.pk} [{self.x:<5}, {self.y:>5}] connected to: slice={self.get_slice()} @ {self.base_station}')
    ...
    def assign_closest_base_station(self, exclude=None):
        if not self.closest_base_stations:
          self.closest_base_stations = [
            (distance((self.x, self.y), (bs.coverage.center[0], bs.coverage.center[1])), bs)
            for bs in self.stat_collector.base_stations
            ]
        updated_list = []
    ...
    def __str__(self):
        s = self.get_slice()
        return f'Client_{self.pk} at ({self.x:.1f}, {self.y:.1f}), connected: {self.connected}, base_station: {self.base_station}, slice: {s}'